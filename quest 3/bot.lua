...
function decideNextAction()
  ...
  local bestBooster = nil  -- Stores the best booster to collect (considering distance)
  ...
  -- Find closest booster within a reasonable range
  for _, booster in pairs(LatestGameState.Boosters or {}) do
    if inRange(player.x, player.y, booster.x, booster.y, 1) then
      if not bestBooster or inRange(player.x, player.y, booster.x, booster.y, 1) < inRange(player.x, player.y, bestBooster.x, bestBooster.y, 1) then
        bestBooster = booster
      end
    end
  end
  ...
  if bestBooster then
    print(colors.blue .. "Booster in range. Collecting." .. colors.reset)
    ao.send({
      Target = Game,
      Action = "CollectBooster",
      Player = ao.id,
      BoosterId = bestBooster.id
    })
